...
const mazes = [
  ...
  // Level 2 – fixed corridors
  [
    {x:0, y:0, w:800, h:20}, {x:0, y:580, w:800, h:20}, {x:0, y:0, w:20, h:600}, {x:780, y:0, w:20, h:600},
    {x:100, y:100, w:600, h:20}, {x:100, y:150, w:20, h:300}, {x:680, y:150, w:20, h:300}, {x:100, y:450, w:600, h:20},
  ],
  // Level 3
  [
    {x:0, y:0, w:800, h:20}, {x:0, y:580, w:800, h:20}, {x:0, y:0, w:20, h:600}, {x:780, y:0, w:20, h:600},
...
function rectCollision(r1,r2){
  const buffer = 1;
  return !(r1.x + r1.w - buffer < r2.x || r1.x + buffer > r2.x + r2.w ||
           r1.y + r1.h - buffer < r2.y || r1.y + buffer > r2.y + r2.h);
}
...
function placeKey(){
  ...
  while(!valid){
    ...
    if(valid && Math.hypot(key.x-player.x,key.y-player.y)<100) valid=false;
  }
  key.collected=false;
}
...
function checkKey(){if(!key.collected && checkCollision(player,key)){key.collected=true;startQuiz();}}
...
function startQuiz(){
  ...
  quizTimer=setInterval(()=>{
    ...
    if(quizTime<=10 && !specialSFX.played){ specialSFX.play().catch(()=>{}); specialSFX.played=true; }
...
function nextLevel(){
  ...
  enemy={x:700,y:500,size:40 + (level-1)*5,speed:2};
  placeKey();
